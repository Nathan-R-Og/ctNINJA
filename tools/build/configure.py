...
gameName = "Chameleon Twist"

DESC = f"{gameName} build.ninja generator"
buildName = "chameleontwist"

repo_cflags = "-fforce-addr"

romLocation = "ver/VERSION/baserom.z64"
...
COMPILER = {
    ...
    "cc": f"{BUILD_TOOLS}/cc/ido5.3/cc",
    "cc_ido": f"{BUILD_TOOLS}/cc/ido5.3/cc",
    "cc_272_dir": f"{BUILD_TOOLS}/cc/gcc2.7.2/",
    "cc_272": f"{BUILD_TOOLS}/cc/gcc2.7.2/gcc",
    "cxx": f"{BUILD_TOOLS}/cc/gcc/g++",
    "CPPFLAGS_COMMON": "-I. -Iinclude -Iinclude/PR -Iassets -Isrc " \
        "-D_LANGUAGE_C -D_FINALROM -DVERSION=$version -DF3DEX_GBI -D_MIPS_SZLONG=32",
    "CPPFLAGS": "",
    "CPPFLAGS_272": "",
    "cflags": f"-c -G0 -O2 -fno-common -B {BUILD_TOOLS}/cc/gcc/",
    "cflags_272": f"-c -G0 -mgp32 -mfp32 -mips3"


}

COMPILER.update(
    {"CPPFLAGS": "-w " + COMPILER["CPPFLAGS_COMMON"] + " -nostdinc"}
)
COMPILER.update(
    {"CPPFLAGS_272": COMPILER["CPPFLAGS_COMMON"] + " -nostdinc"}
)
...
def write_ninja_rules(ninja: ninja_syntax.Writer, cpp: str, cppflags: str, extra_cflags: str, use_ccache: bool,
    ...
    cc = COMPILER["cc"]
    cc_ido = COMPILER["cc_ido"]
    cc_272_dir = COMPILER["cc_272_dir"]
    cc_272 = COMPILER["cc_272"]
    cxx = COMPILER["cxx"]

    CPPFLAGS_COMMON = COMPILER["CPPFLAGS_COMMON"]

    CPPFLAGS = COMPILER["CPPFLAGS"]

    CPPFLAGS_272 = COMPILER["CPPFLAGS_272"]

    cflags = COMPILER["cflags"] + f'{extra_cflags}'
    cflags_272 = COMPILER["cflags_272"] + f'{extra_cflags}'
...
class Configure:
    ...
    def elf_path(self) -> Path:
        ...
        return Path(f"ver/{self.version}/build/" + buildName + ".elf")
    ...
    def linker_script_path(self) -> Path:
        ...
        return Path(f"ver/{self.version}/" + buildName + ".ld")
    ...
    def write_ninja(self, ninja: ninja_syntax.Writer, skip_outputs: Set[str], non_matching: bool, debug: bool):
        ...
        def build(object_paths: Union[Path, List[Path]], src_paths: List[Path], task: str, variables: Dict[str, str] = {}):
            #jump to here when making the rule
        ...
        for entry in self.linker_entries:
            ...
            #checks for if the segment is an offshoot of a segtype
            if isinstance(seg, segtypes.n64.header.N64SegHeader):
                ...
                #if C
                cflags = None
                #these both do the same thing, it just checks if its a dictionary or not
                ...
                # cc
                task = "cc"
                #edit task based on stuff (ccflags)
                if entry.src_paths[0].suffixes[-1] == ".cpp":
                    #not currently ran
                    task = "cxx"

                if "cc_ido" in cflags:
                    #ran once, but made a global flag
                    task += "_ido"
                elif "gcc_272" in cflags:
                    task += "_272"
                #remove any trace
                cflags = cflags.replace("gcc_272", "")
                cflags = cflags.replace("cc_ido", "")
                #dest o path, original c path, task (above), + vars
                ...
                if seg.flip_vertical:
                    flags += "--flip-y "
                #bin path == object path + .bin
                bin_path = entry.object_path.with_suffix(".bin")
                #convert assets/version to build/include
                inc_dir = self.build_path() / "include" / seg.dir
    ...
    def make_current(self, ninja: ninja_syntax.Writer):
        ...
        ninja.build("ver/current/build/" + buildName + ".z64", "phony", str(self.rom_path()))
...
if __name__ == "__main__":
    ...
    #if version defined use
    if len(args.version) > 0:
        versions = args.version
    #else do all versions found
    else:
        versions = []

        for version in VERSIONS:
            romLoc = romLocation.replace("VERSION", version)
            rom = ROOT / romLoc
